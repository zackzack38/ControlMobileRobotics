...
def findPos():
    ...
    for camera in cameras:
        if camera.getRecognitionNumberOfObjects() >= 1:
            for landmark in camera.getRecognitionObjects():
                #take every object into array, array, grab 3 unique
                colorCheck = landmark.get_colors()
                colors[i] = colorCheck
                r[i] = landmark.get_position()
                i = i + 1
    uniqueTemp = []
    k = 0
    uniqueColors = [[],[],[]]
    ...
    for c in colors:
        ...
        if c not in uniqueColors:
            uniqueTemp = c
        if uniqueTemp != [] and k < 3:  
            uniqueColors[k] = uniqueTemp
            rDist[k] = r[n]
            k = k + 1
        n = n + 1
    m = 0
    for x in uniqueColors:
        print("COLORS: " + str(x))
        xPos[m] = getPosObject(x)[0]
        yPos[m] = getPosObject(x)[1]
        m = m + 1 
        """
