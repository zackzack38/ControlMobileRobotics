...
def wallFollow(wall, targetDistance, Kp_side):
    ...
    if(wall == 'l'):
        ...
        if(LD() > 15):
            ...
            rotate(robot, -90)
            driveD(robot, 3, 3.86)
        if((round(FD()) == targetDistance) and (round(LD()) == targetDistance)):
            rotate(robot, 90)
            return
        if(EL == 0):
            print("Forward")
...
while robot.step(timestep) != -1:
    ...
    if(flag_follow == 0 and flag_motion_to_goal == 0):
        ...
        #case 3 wall follow
        print("case 3")
        if camera.getRecognitionNumberOfObjects() == 1 and (obstacle_check(object) == 0) and (SideDistance > 5):
            print("GO TO CASE 1")
            flag_follow = 0
            flag_motion_to_goal = 1
            stopMotion()
            continue
        wallFollow(wallToFollow, 6, 0.02)
